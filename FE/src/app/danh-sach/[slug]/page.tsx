...
import { getComicsByList } from "@/services/comics";
...
export async function generateMetadata({params, searchParams}: {params: {slug: string}, searchParams: {page?: number}}): Promise<Metadata> {
    ...
    const {slug} = params;
    const data: ListTypeProps = await getComicsByList(slug, page);
    if(data === null) {
        notFound();
    ...
    return {
      metadataBase: new URL(`${process.env.NEXT_PUBLIC_BASE_URL}/danh-sach/${slug}`),
...
async function Page({params, searchParams}: {params: {slug: string}, searchParams: {page?: number}}) {
    const page = searchParams.page || 1;
    const data: ListTypeProps = await getComicsByList(params.slug, page);
    if(data === null) {
        notFound();
    }else if(data.status === "error") {
        notFound();
